...
namespace Landmarks.Scripts.Progress.UI
{
    public class LM_EditorUtility : EditorWindow
    {
        ...
        [MenuItem("EditorUtility/Assign Uid to ExperimentTask")]
        private static void AssignUid()
        {
            var usedIds = new HashSet<uint>();

            // Get all the game objects in the scene
            if (!(FindObjectsOfType(typeof(ExperimentTask)) is ExperimentTask[] tasks)) return;
            foreach (var task in tasks)
            {
                var renew = true;
                // Try get the Uid component and if it exists add the ID member of Uid to the usedIds hashset
                if (task.gameObject.TryGetComponent<Uid>(out var uid))
                {
                    if (uid.ID != 0)
                    {
                        usedIds.Add(uid.ID);
                ...
                {
                    uid = task.gameObject.AddComponent<Uid>();
                }
                ...
                // Set Undo for the Uid component
                Undo.RegisterCompleteObjectUndo(uid, "Assign Uid");
